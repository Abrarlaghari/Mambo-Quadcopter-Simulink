...
inputdata1 = inputdata.data(38,1);
inputdata2 = inputdata.data(38,2);
...
Epsilon_new_L = Epsilon_old_L + 0 ;

display ('After First Iteration Epsilon is ');
...
% disp(MaxError2);
  ...
  MaxError2 = 0;
    ...
    if ((MaxError1 >= 20.2 || MaxError1 <= 19.8) || (MaxError2 >= .3 || MaxError2 <= -.3))
        disp('Adaptivity Required');
   % pathgen(simout.time, simout.data(:,1),simout.data(:,2),simout.data(:,3),simout.data(:,4),simout.data(:,5),simout.data(:,6));
        flag_adap = 1;
    else
