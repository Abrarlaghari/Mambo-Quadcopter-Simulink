function [delta1, delta2] = PlottingT2DMF_a_change (IP1, EP, ind , nu, Noise)
...
for i=1:length(x)
    ...
    deltal1(i) = abs( ((x(i)-IP1(j))/ epl1(j) ) * (1 / (1 + exp(100*((x(i)-IP1(j))) ))) )^4;
    deltal2(i) = abs( (((x(i)-IP1(j))-Noise)/ (epl1(j)) ) * (1 / (1 + exp(100*((x(i)-IP1(j))- Noise) ))) )^4;
...
%   nulterm = 1;
    ...
    deltar1(i) = abs( ((x(i)-IP1(j))/ epr1(j) ) * (1 / (1 + exp(-100*((x(i)-IP1(j))) ))))^4; 
    deltar2(i) = abs( (((x(i)-IP1(j))+ Noise)/ (epr1(j)) ) * (1 / (1 + exp(-100*((x(i)-IP1(j))+ Noise) ))))^4; 
